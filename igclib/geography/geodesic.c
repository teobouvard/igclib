...
void geod_init(struct geod_geodesic *g, real a, real f)
{
  ...
  g->b = g->a * g->f1;
  g->c2 = (sq(g->a) + sq(g->b) *
                          (g->e2 == 0 ? 1 : (g->e2 > 0 ? atanhx(sqrt(g->e2)) : atan(sqrt(-g->e2))) / sqrt(fabs(g->e2)))) /
                      ...
                      real *pS12)
{
  ...
  if (outmask & GEOD_LONGITUDE)
  {
    ...
    omg12 = (flags & GEOD_LONG_UNROLL)
                ? E * (sig12 - (atan2(ssig2, csig2) - atan2(l->ssig1, l->csig1)) + (atan2(E * somg2, comg2) - atan2(E * l->somg1, l->comg1)))
                : atan2(somg2 * l->comg1 - comg2 * l->somg1,
                        comg2 * l->comg1 + somg2 * l->somg1);
    lam12 = omg12 + l->A3c *
                        (sig12 + (SinCosSeries(TRUE, ssig2, csig2, l->C3a, nC3 - 1) - l->B31));
    lon12 = lam12 / degree;
    lon2 = (flags & GEOD_LONG_UNROLL) ? l->lon1 + lon12 : AngNormalize(AngNormalize(l->lon1) + AngNormalize(lon12));
  }
                      ...
                      unsigned caps)
{
  ...
  if (caps & (OUT_ALL & GEOD_DISTANCE_IN))
    caps |= GEOD_DISTANCE;
  geod_lineinit_int(l, g, lat1, lon1, azi1, salp1, calp1, caps);
  geod_setarc(l, a12);
}
                  ...
                  real Ca[])
{
  ...
  else
  {
    ...
    { /* f < 0 */
      ...
      betscale = x < -(real)(0.01) ? sbet12a / x : -g->f * sq(cbet1) * pi;
              ...
              real Ca[])
{
  ...
  calp2 = cbet2 != cbet1 || fabs(sbet2) != -sbet1 ? sqrt(sq(calp1 * cbet1) +
